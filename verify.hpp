...
template<typename T>
inline T getInt(std::vector<u8> buffer, int pos) {
    return *reinterpret_cast<T*>(buffer.data() + pos);
}

// CRC32 poly8 table
const u32 crc32_table[] = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba,
...
bool verifyCRC32(std::vector<u8> buffer, int position, u32 length, u32 crc);
